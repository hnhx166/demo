...
import com.chnghx.web.auth.constant.ConstantKey;
...
//            }
            ...
            if (!validateOAuth2AppKey(oauthRequest)){
                OAuthResponse oauthResponse = OAuthASResponse
                                              ...
                                              .buildJSONMessage();
                logger.error("oauthRequest.getRedirectURI() : "+oauthRequest.getRedirectURI()+" oauthResponse.getBody() : "+oauthResponse.getBody());
                model.addObject("errorMsg", oauthResponse.getBody());
                model.setViewName("/oauth2/error");
                return model;
            }
            //查询客户端Appkey应用的信息
            ...
            model.addObject("response_type",oauthRequest.getResponseType());
            model.addObject("client_id",oauthRequest.getClientId());
            model.addObject("redirect_uri",oauthRequest.getRedirectURI());
            System.out.println(oauthRequest.getScopes());
            Set<String> socpes = oauthRequest.getScopes();
            Iterator<String> it = socpes.iterator();
            String socpe = "";
            if(it.hasNext()){
            	socpe += it.next() + ",";
            }
            socpe = socpe.substring(0, socpe.lastIndexOf(","));
//            model.addObject("scope",oauthRequest.getScopes());
            model.addObject("scope", socpe);
            //验证用户是否已登录
            if(session.getAttribute(ConstantKey.MEMBER_SESSION_KEY)==null) {
                //用户登录
                if(!validateOAuth2Pwd(request)) {
                    //登录失败跳转到登陆页
                    model.setViewName("/oauth2/login");
                    return model;
                }
            }
            //判断此次请求是否是用户授权
            if(request.getParameter("action")==null||!request.getParameter("action").equalsIgnoreCase("authorize")){
                //到申请用户同意授权页
            	model.setViewName("/oauth2/authorize");
                return model;
            }
           //生成授权码 UUIDValueGenerator OR MD5Generator
           String authorizationCode = new OAuthIssuerImpl(new MD5Generator()).authorizationCode();
           //把授权码存入缓存
           VCache.put(authorizationCode, DigestUtils.sha1Hex(oauthRequest.getClientId()+oauthRequest.getRedirectURI()), 600000);
           //构建oauth2授权返回信息
           OAuthResponse oauthResponse = OAuthASResponse
           ...
           System.out.println(oauthResponse.getLocationUri());
           //申请令牌成功重定向到客户端页
